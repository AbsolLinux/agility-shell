...
class WallpaperDrawer:
    def __init__(self, **kwargs):
        ...
        self._wheel = WallpaperWheelWindow(**kwargs)
        self._coverflow = WallpaperCoverflowWindow(**kwargs)
    ...
    def _get_active_subwindow(self):
        ...
        if style == "stairs":
            ...
            return self._coverflow
        return self._dock

    def is_visible(self) -> bool:
        return (
            self._dock.is_visible()
            or self._stairs.is_visible()
            or self._mesh.is_visible()
            or self._wheel.is_visible()
            or self._coverflow.is_visible()
        )
    ...
    def close(self):
        for win in (self._dock, self._stairs, self._mesh, self._wheel, self._coverflow):
            if win.is_visible():
                win.close()
