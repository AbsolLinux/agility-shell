...
class DesktopNightLight(Box):
    def __init__(self, **kwargs):
        ...
        self.icon = Icon(icon_name="moon-stars-duotone", icon_size=24, h_align="center", v_align="center")
        ...
        self.toggle_btn = Button(
            ...
            h_expand=True,
            on_clicked=lambda *_: night_mode.toggle(),
        )
    ...
    def _sync(self, *_):
        is_active = bool(night_mode.enabled)
        if is_active:
            self.progress_bar.value = 100
            self.status_label.set_label("Warm Filter Active")
            self.toggle_btn.set_label("Turn Off")
            self.toggle_btn.add_style_class("active")
        else:
            self.progress_bar.value = 0
            self.status_label.set_label("Natural Color")
